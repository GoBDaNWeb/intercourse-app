...
const Message = memo(({ messageData, lastMessage }) => {
    ...
    const { user_id, author_avatar, author, message, inserted_at } =
        messageData;

    const isFirstMessageByUser =
        !lastMessage || lastMessage.user_id !== user_id;
    ...
    return (
        <div className="relative flex items-start justify-start gap-2 w-full min-h-10 ">
            ...
            <div className="min-h-10 min-w-20 max-w-[70%]">
                ...
                <div
                    className={`flex gap-2 items-end  bg-white p-2 rounded-2xl shadow-custom ${
                        isFirstMessageByUser ? 'mt-6' : ''
                    }`}
                >
                    <div>{message}</div>
